...
namespace DataTron
{
    class KService
    {
        public string kService(string heap)
        {
            string kService = $@"@echo off
SETLOCAL enabledelayedexpansion

TITLE Elasticsearch Service 2.3.3

if NOT DEFINED KCURA_JAVA_HOME goto err
...
if not exist ""%KCURA_JAVA_HOME%\bin\java.exe"" (
echo KCURA_JAVA_HOME points to an invalid Java installation (no java.exe found in ""%KCURA_JAVA_HOME%""^). Exiting...
goto:eof
)

""%KCURA_JAVA_HOME%\bin\java"" -Xmx50M -version > nul 2>&1
...
""%KCURA_JAVA_HOME%\bin\java"" -Xmx50M -version 2>&1 | ""%windir%\System32\find"" ""64-Bit"" >nul:
...
echo Using KCURA_JAVA_HOME (%ARCH%):  ""%KCURA_JAVA_HOME%""

rem Check JVM server dll first
set JVM_DLL=%KCURA_JAVA_HOME%\jre\bin\server\jvm.dll
if exist ""%JVM_DLL%"" goto foundJVM

rem Check 'server' JRE (JRE installed on Windows Server)
set JVM_DLL=%KCURA_JAVA_HOME%\bin\server\jvm.dll
if exist ""%JVM_DLL%"" goto foundJVM

rem Fallback to 'client' JRE
set JVM_DLL=%KCURA_JAVA_HOME%\bin\client\jvm.dll

if exist ""%JVM_DLL%"" (
echo Warning: KCURA_JAVA_HOME points to a JRE and not JDK installation; a client (not a server^) JVM will be used...
) else (
echo KCURA_JAVA_HOME points to an invalid Java installation (no jvm.dll found in ""%KCURA_JAVA_HOME%""^). Existing...
goto:eof
)

:foundJVM
if ""%ES_MIN_MEM%"" == """" set ES_MIN_MEM = {heap}g
if ""%ES_MAX_MEM%"" == """" set ES_MAX_MEM = {heap}g
...
if %SERVICE_ID% == elasticsearch-service-x86 set SQLAUTH_DIR=%ES_HOME%\sqlauth\x86
if %SERVICE_ID% == elasticsearch-service-x64 set SQLAUTH_DIR=%ES_HOME%\sqlauth\x64
if ""%SQLAUTH_DIR%"" == """" set SQLAUTH_DIR=%ES_HOME%\sqlauth\x64

   set ES_PARAMS=-Delasticsearch;-Des.path.home=""%ES_HOME%"";-Des.default.path.home=""%ES_HOME%"";-Des.default.path.logs=""%LOG_DIR%"";-Des.default.path.data=""%DATA_DIR%"";-Des.default.path.conf=""%CONF_DIR%"";-Djava.library.path=""%SQLAUTH_DIR%""
...
:err
echo KCURA_JAVA_HOME environment variable must be set!
pause
goto:eof
